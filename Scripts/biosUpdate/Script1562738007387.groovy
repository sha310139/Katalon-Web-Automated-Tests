...
for (int i = 0; i < 4; i++) {
    ...
    smartWaitAndClick(findTestObject('BIOSandBMCUpdate/Page_ARMOR/button_Update BIOS'), GlobalVariable.G_Small_Timeout)

    smartWaitAndClick(findTestObject('BIOSandBMCUpdate/Page_ARMOR/input_Update BIOS firmware via BMC_update-bios-protocol'), GlobalVariable.G_Small_Timeout)
	...
	if (!WebUI.getText(findTestObject('BIOSandBMCUpdate/Page_ARMOR/messege')).equals('Update BIOS Firmware Complete') ) {
		return false
	} // if
